...
fn serve(mut stream: TcpStream) -> std::io::Result<()> {
    ...
    let (status, media_type, body) = match first_line.split_whitespace().nth(1) {
        Some("/") => ("200 OK", "text/html", HOME),
        Some("/metadata") => ("200 OK", "text/html", METADATA),
        Some("/robots.txt") => ("200 OK", "text/plain", ROBOTS),
        Some("/sitemap.xml") => ("200 OK", "application/xml", SITEMAP),
